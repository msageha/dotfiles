...
- コメント・docstring は日本語で書くが、技術用語は原語のままとし、無理に日本語の単語へ訳さない (pipeline を「段」、table を「表」と書かない)。例外メッセージ・ログは英語。詳細は AGENTS.md のコード規約に従う。
- エラーは握りつぶさず適切に扱う (try/catch・error boundary)。
