...
- コメント・docstring・コード内の言語選択は AGENTS.md のコード規約に従う (自明コメントを書かない・提出前のコメント点検・技術用語は原語のまま・機械向け文字列は英語)。
...
- **codex**: アーキテクチャ相談、設計パターン、複雑なコード生成のレビュー。まとまったコード・文書の変更は、納品前に codex レビューを自律的に一巡取り、妥当な指摘を反映する (typo 修正等の軽微な変更は除く)。ユーザーがレビューを明示依頼した場合は指摘が無くなるまで修正と再レビューを反復する。
